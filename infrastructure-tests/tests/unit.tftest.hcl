...
assert {
	condition = startswith(aws_instance.server.tags.Name, "IATF")
        error_message = "Instance Name did not start with IATF"
}
assert {
	condition = length(aws_instance.server.tags.Name) <= 25
        error_message = "Instance Name is greater than 25 chars "
}
...
assert {
	condition = length(aws_instance.server.tags.Name) <= 25
        error_message = "Instance Name  is greater than 20 chars "
}
}
